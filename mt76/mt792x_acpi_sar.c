...
static int
mt7902_mt792x_asar_acpi_read_mtcl(struct mt7902_mt792x_dev *dev, u8 **table, u8 *version)
{
	int ret;

	*version = ((ret = mt7902_mt792x_acpi_read(dev, MT792x_ACPI_MTCL, table, NULL)) < 0)
		   ? 1 : 2;
...
static int
mt7902_mt792x_asar_acpi_read_mtds(struct mt7902_mt792x_dev *dev, u8 **table, u8 version)
{
	int len, ret, sarlen, prelen, tblcnt;
	bool enable;

	ret = mt7902_mt792x_acpi_read(dev, MT792x_ACPI_MTDS, table, &len);
	if (ret)
		return ret;
	...
	switch (version) {
	case 1:
		enable = ((struct mt7902_mt792x_asar_dyn *)*table)->enable;
		sarlen = sizeof(struct mt7902_mt792x_asar_dyn_limit);
		prelen = sizeof(struct mt7902_mt792x_asar_dyn);
		break;
	case 2:
		enable = ((struct mt7902_mt792x_asar_dyn_v2 *)*table)->enable;
		sarlen = sizeof(struct mt7902_mt792x_asar_dyn_limit_v2);
		prelen = sizeof(struct mt7902_mt792x_asar_dyn_v2);
		break;
	default:
		return -EINVAL;
	}
...
static int
mt7902_mt792x_asar_acpi_read_mtgs(struct mt7902_mt792x_dev *dev, u8 **table, u8 version)
{
	int len, ret, sarlen, prelen, tblcnt;

	ret = mt7902_mt792x_acpi_read(dev, MT792x_ACPI_MTGS, table, &len);
	if (ret)
		return ret;
	...
	switch (version) {
	case 1:
		sarlen = sizeof(struct mt7902_mt792x_asar_geo_limit);
		prelen = sizeof(struct mt7902_mt792x_asar_geo);
		break;
	case 2:
		sarlen = sizeof(struct mt7902_mt792x_asar_geo_limit_v2);
		prelen = sizeof(struct mt7902_mt792x_asar_geo_v2);
		break;
	default:
		return -EINVAL;
	}
...
/* MTFG : Flag Table */
static int
mt7902_mt792x_asar_acpi_read_mtfg(struct mt7902_mt792x_dev *dev, u8 **table)
{
	int len, ret;

	ret = mt7902_mt792x_acpi_read(dev, MT792x_ACPI_MTFG, table, &len);
	if (ret)
		return ret;
...
{
	struct mt7902_mt792x_acpi_sar *asar;
	int ret;
	...
	ret = mt7902_mt792x_asar_acpi_read_mtcl(dev, (u8 **)&asar->countrylist, &asar->ver);
	if (ret) {
		devm_kfree(dev->mt76.dev, asar->countrylist);
		asar->countrylist = NULL;
	}

	ret = mt7902_mt792x_asar_acpi_read_mtds(dev, (u8 **)&asar->dyn, asar->ver);
	if (ret) {
	...
	/* MTGS is optional */
	ret = mt7902_mt792x_asar_acpi_read_mtgs(dev, (u8 **)&asar->geo, asar->ver);
	if (ret) {
	...
	/* MTFG is optional */
	ret = mt7902_mt792x_asar_acpi_read_mtfg(dev, (u8 **)&asar->fg);
	if (ret) {
...
EXPORT_SYMBOL_GPL(mt7902_mt792x_init_acpi_sar);

static s8
mt7902_mt792x_asar_get_geo_pwr(struct mt7902_mt792x_phy *phy,
			enum nl80211_band band, s8 dyn_power)
{
	struct mt7902_mt792x_acpi_sar *asar = phy->acpisar;
	struct mt7902_mt792x_asar_geo_band *band_pwr;
	s8 geo_power;
	u8 idx, max;
...
static s8
mt7902_mt792x_asar_range_pwr(struct mt7902_mt792x_phy *phy,
		      ...
		      u8 idx)
{
	...
	return mt7902_mt792x_asar_get_geo_pwr(phy, band, limit[idx]);
}
...
int mt7902_mt792x_init_acpi_sar_power(struct mt7902_mt792x_phy *phy, bool set_default)
{
	...
	if (!phy->acpisar || !((struct mt7902_mt792x_acpi_sar *)phy->acpisar)->dyn)
		return 0;
	...
	for (i = 0; i < capa->num_freq_ranges; i++) {
		...
		frp->power = min_t(s8, set_default ? 127 : frp->power,
				   mt7902_mt792x_asar_range_pwr(phy, frp->range, i));
	}

	return 0;
}
EXPORT_SYMBOL_GPL(mt7902_mt792x_init_acpi_sar_power);

u8 mt7902_mt792x_acpi_get_flags(struct mt7902_mt792x_phy *phy)
{
	struct mt7902_mt792x_acpi_sar *acpisar = phy->acpisar;
	struct mt7902_mt792x_asar_fg *fg;
	struct {
		u8 acpi_idx;
		u8 chip_idx;
	} map[] = {
...
EXPORT_SYMBOL_GPL(mt7902_mt792x_acpi_get_flags);

static u8
mt7902_mt792x_acpi_get_mtcl_map(int row, int column, struct mt7902_mt792x_asar_cl *cl)
{
	u8 config = 0;
	u8 mode_6g, mode_5g9;
...
u8 mt7902_mt792x_acpi_get_mtcl_conf(struct mt7902_mt792x_phy *phy, char *alpha2)
{
	...
	struct mt7902_mt792x_acpi_sar *sar = phy->acpisar;
	struct mt7902_mt792x_asar_cl *cl;
	int col, row, i;
	...
	for (i = 0; i < ARRAY_SIZE(cc_list_all); i++) {
		...
		if (!memcmp(cc_list_all[i], alpha2, 2))
			return mt7902_mt792x_acpi_get_mtcl_map(row, col, cl);
	}

	for (i = 0; i < ARRAY_SIZE(cc_list_eu); i++)
		if (!memcmp(cc_list_eu[i], alpha2, 2))
			return mt7902_mt792x_acpi_get_mtcl_map(0, 6, cl);

	return mt7902_mt792x_acpi_get_mtcl_map(0, 7, cl);
}
EXPORT_SYMBOL_GPL(mt7902_mt792x_acpi_get_mtcl_conf);
