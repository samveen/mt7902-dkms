...
static void mt7902_reg_remap_restore(struct mt7902_mt792x_dev *dev)
{
	/* remap to ori status */
...
static u32 mt7902_reg_map_l1(struct mt7902_mt792x_dev *dev, u32 addr)
{
...
static u32 mt7902_reg_map_l2(struct mt7902_mt792x_dev *dev, u32 addr)
{
...
static int mt7902_dma_init(struct mt7902_mt792x_dev *dev)
{
	...
	/* init tx queue */
	ret = mt7902_mt76_connac_init_tx_queues(dev->phy.mt76, MT7925_TXQ_BAND0,
					 MT7925_TX_RING_SIZE,
	...
	/* command to WM */
	ret = mt7902_mt76_init_mcu_queue(&dev->mt76, MT_MCUQ_WM, MT7925_TXQ_MCU_WM,
				  MT7925_TX_MCU_RING_SIZE, MT_TX_RING_BASE);
	if (ret)
		return ret;

	/* firmware download */
	ret = mt7902_mt76_init_mcu_queue(&dev->mt76, MT_MCUQ_FWDL, MT7925_TXQ_FWDL,
			    ...
			    const struct pci_device_id *id)
{
	...
	mt7902_mt76_rmw_field(dev, MT_HW_EMI_CTL, MT_HW_EMI_CTL_SLPPROT_EN, 1);
...
static int mt7902_pci_suspend(struct device *device)
{
	...
	mt7902_mt76_wr(dev, MT_WFDMA0_HOST_INT_DIS,
