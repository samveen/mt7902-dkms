...
EXPORT_SYMBOL_GPL(mt7902_mt792x_mac_work);
...
EXPORT_SYMBOL_GPL(mt7902_mt792x_mac_set_timeing);
...
EXPORT_SYMBOL_GPL(mt7902_mt792x_mac_update_mib_stats);
				     ...
				     bool unicast)
{
	...
	sta = container_of(wcid, struct mt7902_mt792x_sta, wcid);
	if (!sta->vif)
		return NULL;

	return &sta->vif->sta.wcid;
}
EXPORT_SYMBOL_GPL(mt7902_mt792x_rx_get_wcid);

static void
mt7902_mt792x_mac_rssi_iter(void *priv, u8 *mac, struct ieee80211_vif *vif)
{
...
void mt7902_mt792x_mac_assoc_rssi(struct mt7902_mt792x_dev *dev, struct sk_buff *skb)
{
	...
	ieee80211_iterate_active_interfaces_atomic(mt76_hw(dev),
		...
		mt7902_mt792x_mac_rssi_iter, skb);
}
EXPORT_SYMBOL_GPL(mt7902_mt792x_mac_assoc_rssi);
...
EXPORT_SYMBOL_GPL(mt7902_mt792x_mac_reset_counters);

static u8
mt7902_mt792x_phy_get_nf(struct mt7902_mt792x_phy *phy, int idx)
{
	return 0;
}

static void
mt7902_mt792x_phy_update_channel(struct mt76_phy *mphy, int idx)
{
	struct mt7902_mt792x_dev *dev = container_of(mphy->dev, struct mt7902_mt792x_dev, mt76);
	...
	nf = mt7902_mt792x_phy_get_nf(phy, idx);
	if (!phy->noise)
		phy->noise = nf << 4;
	else if (nf)
...
void mt7902_mt792x_update_channel(struct mt76_phy *mphy)
{
	...
	mt7902_mt792x_phy_update_channel(mphy, 0);
	/* reset obss airtime */
	mt76_set(dev, MT_WF_RMAC_MIB_TIME0(0), MT_WF_RMAC_MIB_RXTIME_CLR);
	mt76_connac_power_save_sched(mphy, &dev->pm);
}
EXPORT_SYMBOL_GPL(mt7902_mt792x_update_channel);
...
EXPORT_SYMBOL_GPL(mt7902_mt792x_reset);
...
EXPORT_SYMBOL_GPL(mt7902_mt792x_mac_init_band);
...
void mt7902_mt792x_pm_wake_work(struct work_struct *work)
{
	...
	if (!mt7902_mt792x_mcu_drv_pmctrl(dev)) {
...
EXPORT_SYMBOL_GPL(mt7902_mt792x_pm_wake_work);
...
void mt7902_mt792x_pm_power_save_work(struct work_struct *work)
{
	...
	if (!mt7902_mt792x_mcu_fw_pmctrl(dev)) {
...
EXPORT_SYMBOL_GPL(mt7902_mt792x_pm_power_save_work);
